...
let currentData = {};
currentData[dpKeys.brightness] = "pending"; // Brightness (25,50,75,100)
currentData[dpKeys.mode] = "pending"; // Mode ("OFF","VEG","BLOOM","FULL")
currentData[dpKeys.humidity] = "pending"; // Humidity (%)
currentData[dpKeys.temperature] = "pending"; // Temperature (ºC*10)
...
device.on("data", data => {
    ...
    process.send({
        "type": "newData",
        "message": {
            "brightness": getCanonicalBrightness(), // Brightness (25,50,75,100)
...
process.on("message", (data) => {
    switch (data.type) {
        case "set":
            switch (data.key) {
                case "veg":
                    if (currentData[dpKeys.mode] == "pending") { setMode("VEG"); }
                    ...
                    break;
                case "bloom":
                    if (currentData[dpKeys.mode] == "pending") { setMode("BLOOM"); }
...
function getCanonicalBrightness() {
    if (currentData[dpKeys.brightness] == "pending") return "pending";
    return currentData[dpKeys.brightness];
};
function isVegLampOn() {
    if (currentData[dpKeys.mode] == "pending") return "pending";
...
function getCanonicalHumidity() {
    if (currentData[dpKeys.humidity] == "pending") return "pending";
    return currentData[dpKeys.humidity];
};
function getCanonicalTemperature() {
    if (currentData[dpKeys.temperature] == "pending") return "pending";
