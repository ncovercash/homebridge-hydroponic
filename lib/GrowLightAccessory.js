...
class GrowLightAccessory {
    ...
    setupListener() {
        ...
        this.listener = fork(__dirname+"/GrowLightListener.js", [this.id, this.key], { execArgv: ["--unhandled-rejections=strict"] });
            ...
            switch (data.type) {
                ...
                case "newData":
                    for (let key in data.message) {
                        if (data.message[key] != "pending" && data.message[key] != null) {
                            this.currentData[key] = data.message[key];
                        }
                    }
    ...
    initializeCharacteristics() {
        ...
        this.characteristics.vegOn.on("get", callback => callback(this.shouldConnectionError(this.currentData.veg), this.currentData.veg));
        ...
        this.characteristics.vegBrightness.on("get", callback => callback(this.shouldConnectionError(this.currentData.brightness), this.currentData.brightness));
        ...
        this.characteristics.bloomOn.on("get", callback => callback(this.shouldConnectionError(this.currentData.bloom), this.currentData.bloom));
        ...
        this.characteristics.bloomBrightness.on("get", callback => callback(this.shouldConnectionError(this.currentData.brightness), this.currentData.brightness));
        ...
        this.characteristics.humidity.on("get", callback => callback(this.shouldConnectionError(this.currentData.humidity), this.currentData.humidity));
        ...
        this.characteristics.temperature.on("get", callback => callback(this.shouldConnectionError(this.currentData.temperature), this.currentData.temperature));
    ...
    shouldConnectionError(associatedData) {
        if (!this.currentData.connected) {
            this.log.warn("Returning no response");
            return new Error("Device not connected");
        }
        if (associatedData == "pending") {
            this.log.info("Get is still pending initial data...");
